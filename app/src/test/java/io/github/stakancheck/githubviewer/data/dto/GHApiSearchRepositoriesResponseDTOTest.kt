...
class GHApiSearchRepositoriesResponseDTOTest {
    ...
    @Test
    fun `test deserialization of GHApiSearchRepositoriesResponseDTO`() {
        val jsonResponse = """
        {
          "total_count": 40,
        ...
        val response = json.decodeFromString<GHApiSearchRepositoriesResponseDTO>(jsonResponse)

        assertEquals(40, response.totalCount)
        assertEquals(false, response.incompleteResults)
        assertEquals(1, response.items.size)

        val repo = response.items[0]
        assertEquals("Tetris", repo.name)
        assertEquals(3081286, repo.id)
        assertEquals(
        ...
        assertEquals("2012-01-01T00:31:50Z", repo.createdAt)
    }
}
